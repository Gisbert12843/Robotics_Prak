        ...
        for i in range(x, x + w, rect_size):
            ...
            for j in range(y, y + h, rect_size):
                ...
                #                                         and between V_RES*0.99 and V_RES*1.01 in y-axis (j value)
                    ...
                    if(abs(V_RES - pol_centr[0]) < abs(V_RES - closest_polygon[0])):
                        closest_polygon = pol_centr
                        # print(f"Comparing pol_centr: {pol_centr} with ranges x: [155, 165], y: [180, 190]")
                        if pol_centr[0] >= H_RES_HALF*0.99 and pol_centr[0] <= H_RES_HALF*1.01 and pol_centr[1] >= V_RES*0.99 and pol_centr[1] <= V_RES*1.01:
                          middle_anchor = True
                          break
                        elif pol_centr[0] >= H_RES_HALF*0.99 and pol_centr[0] <= H_RES_HALF*1.01: #here we can skip the y-axis check, because we are already in the x-axis range that isnt a middle_anchor candidate
                          right_anchor = True
                          left_anchor = True

                        elif i > H_RES_HALF*1.01:
                          right_anchor = True
                          left_anchor = False
